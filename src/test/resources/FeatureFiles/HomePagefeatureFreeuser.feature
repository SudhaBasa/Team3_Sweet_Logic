...
Background: User is in password auth page 
 ...
 @OrderNavigationBar
  Scenario Outline: Verify order of text in navigation bar
    Given User is in password auth page for FreeUser
    When User clicks sign in after entering password
    Then User should see the navigation bar displaying items in the order "<HomeTabs>" in HomePage Tab

Examples:
|HomeTabs|
|Home, Dashboard, Education |
...
@TitleExcercise
  Scenario: Verify the title in Exercise section
    ...
    Then User should see title "Today's Exercise Plan" in Excercise Tab
